...
class ElectricCar(Vehicle):
    ...
    def __str__(self):
        status = "rented" if self.is_rented else "available"
        return (
            f"Electric Car: {self.make} {self.model} "
            f"({self.plate}) [{status}], Battery: {self.battery_kwh} kWh"
        )
...
class Motorbike(Vehicle):
    ...
    def __str__(self):
        status = "rented" if self.is_rented else "available"
        return (
            f"Motorbike: {self.make} {self.model} "
            f"({self.plate}) [{status}], Engine: {self.engine_cc} cc"
        )
